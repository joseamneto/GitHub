...
namespace Sitecore.Foundation.SitecoreExtensions.Extensions
{
    ...
    public static class ItemExtensions
    {
        ...
        public static void PublishNow(this Item item, Sitecore.Publishing.PublishMode publishMode, bool deep)
        {
            Assert.IsNotNull(item, "Item cannot be null");
            Assert.ArgumentCondition(item.Database.Name == "master", "item", "Item must be from master database");
            ...
            Sitecore.Publishing.PublishOptions publishOptions =
                new Sitecore.Publishing.PublishOptions(sourceDatabase,
                                                        targetDatabase,
                                                        publishMode,
                                                        item.Language,
                                                        System.DateTime.Now);
            ...
            Sitecore.Diagnostics.Log.Debug("[ItemExtensions] Published item, mode: " + publishMode + ", deep: " + deep + ", path: " + item.Paths.FullPath, typeof(ItemExtensions));
        }

        public static bool InstanceOfTemplate(this Item item, string templateName)
        {
            bool flag;
            if (item != null && !string.IsNullOrEmpty(item.TemplateName) && !string.IsNullOrEmpty(templateName))
            {
                if (templateName.Equals(item.TemplateName))
                {
                    flag = true;
                }
                else
                {
                    flag = item.GetBaseTemplates().Any(t => t.IsTemplate(templateName));
                }
            }
            else
            {
        ...
        public static bool DescendsFrom(this Item item, ID templateId)
        {
        ...
        public static bool InstanceOfTemplate(this Item item, string[] templateNames)
        {
            bool flag = false;
            IEnumerable<TemplateItem> templateItems;

            if (item != null && templateNames != null)
            {
                templateItems = item.GetBaseTemplates();
                flag = templateItems.Where(t => t.IsTemplate(templateNames)).Count() > 0;
            }

            return flag;
        }

        public static IEnumerable<TemplateItem> GetBaseTemplates(this Item item)
        {
            return TemplateItemExtension.GetBaseTemplates(item);
        }
    }
